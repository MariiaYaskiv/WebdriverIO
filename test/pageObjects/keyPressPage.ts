...
export default class KeyPress extends RootObject {
    ...
    public open() {
        AllureReporter.addStep('Verify if the user can enter numbers in the input')
        return super.open('/key_presses');
    }

    async setValueOnTheInput(text: string, options?: stepOptions
    ): Promise<void> {
    ...
    async expectedResult(text: string, options?: stepOptions
    ): Promise<void> {
