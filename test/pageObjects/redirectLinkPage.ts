...
export default class RedirectLink extends RootObject {
    ...
    public open() {
        AllureReporter.addStep('Verify if the user can click on the link and follow the page with statuscodes')
        return super.open('/redirector');
    }


    async redirectLinkClick(status: string, options?: stepOptions
    ): Promise<void> {
