...
export default class FormAuthentication extends RootObject {
    ...
    public open() {
        AllureReporter.addStep('Verify if the user can put his values on the input and enter to the web-site')
    ...
    async textContaining(text?: string, options?: stepOptions,
    ): Promise<void> {
    ...
    async putUsernameAndPassword(username?: string, password?: string, options?: stepOptions,
    ): Promise<void> {
        await step(
            options,
