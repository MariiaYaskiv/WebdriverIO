...
export default class Inputs extends RootObject {
    ...
    public open() {
        AllureReporter.addStep('Verify if the user can enter the numbers in the input and the input could be focused')
        return super.open('/inputs');
    }

    async inputIsFocused(string: string, options?: stepOptions
    ): Promise<void> {
