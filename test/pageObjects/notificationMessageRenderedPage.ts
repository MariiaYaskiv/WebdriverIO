...
export default class NotificationMessageRendered extends RootObject {
    ...
    public open() {
        AllureReporter.addStep('Verify if the user can click on the button and receive the comparing massage')
        return super.open('/notification_message_rendered');
    }


    async clickButton(options?: stepOptions
    ): Promise<void> {
    ...
    async textMessage(options?: stepOptions
    ): Promise<void> {
